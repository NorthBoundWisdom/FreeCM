...
export class LockModeController {
  ...
  async runLockWorkflowCommand(command: LockWorkflowCommand): Promise<void> {
    ...
    let targetFolder: Parameters<CommandControllerHost["terminalOutput"]>[0] | undefined;
    try {
      ...
      if (command === "usePinned") {
        this.host.logToTerminal("info", "Use pinned: updating active lock.", folder);
        await usePinned(folder.fsPath, { output: this.host.terminalOutput(folder) });
        this.host.logToTerminal("success", "Active lock now uses pinned dependencies.", folder);
      } else if (command === "manualAll") {
        this.host.logToTerminal("info", "Manual all: updating active lock.", folder);
        await manualAll(folder.fsPath, { output: this.host.terminalOutput(folder) });
        this.host.logToTerminal("success", "Active lock now uses manual seed paths.", folder);
      } else if (command === "pinLatest") {
        this.host.logToTerminal("info", "Pin latest: switching active lock to latest.", folder);
        await pinLatest(
          folder.fsPath,
          (repoRoot) => runOfflineUpdate(repoRoot, this.host.terminalOutput(folder)),
          { output: this.host.terminalOutput(folder) },
        );
        this.host.logToTerminal("success", "Active lock pinned latest local seed commits.", folder);
      } else {
